...
public class lab2 implements ParamMarioLevelGenerator {
    ...
    public ArrayList < float [] > getParameterSearchSpace() {
        ...
        searchSpace.add(new float[]{10,8});//OBSTACLES_LOCATION
        searchSpace.add(new float[]{0.1f,0.2f});//OBSTACLES_PROB
        searchSpace.add(new float[]{3,4});//COLLECTIBLE_LOCATION
        searchSpace.add(new float[]{0.05f,0.04f});//COLLECTIBLE_PROB
    ...
    public void setParameters ( int [] paramIndex ) {
        ...
        for(int i = 0; i< paramIndex.length; i++){
            ...
            this.GROUND_Y_LOCATION = (int) searchSpace.get(0)[index];
            this.GROUND_PROB = searchSpace.get(1)[index];
            this.OBSTACLES_LOCATION = (int) searchSpace.get(2)[index];
            this.OBSTACLES_PROB = searchSpace.get(3)[index];
            this.COLLECTIBLE_LOCATION = (int) searchSpace.get(4)[index];
            this.COLLECTIBLE_PROB = searchSpace.get(5)[index];
            this.ENEMY_PROB = searchSpace.get(6)[index];
            this.FLOOR_PADDING = (int) searchSpace.get(7)[index];
        }
    }
